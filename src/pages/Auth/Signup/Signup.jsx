...
const Signup = () => {
  ...
  const formhandlesubmit = async (e) => {
    ...
    if (!isSignUpFormFilled()) {
      toast.error("Please fill all the required fields", { theme: "dark" });
      return;
    }

    if (confirmpwd !== password) {
      toast.error("Passwords are not same", { theme: "dark" });
    ...
    try {
      ...
      if (response.data.message !== "OTP verified successfully") {
        toast.error("Wrong OTP. Please try again", { theme: "dark" });
        return;
      }
    } catch (error) {
      toast.error("Wrong OTP. Please try again", { theme: "dark" });
    ...
    axios
      ...
      .catch((error) => {
        ...
        if (error.response?.data.error === "Email already exists") {
          toast.info("Email already exists", { theme: "dark" });
        } else if (
  ...
  const sendOTP = async (e) => {
    ...
    if (!emailPattern.test(email)) {
      toast.error("Please enter a valid email id", { theme: "dark" });
      return;
    }

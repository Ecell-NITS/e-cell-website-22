...
const Adovation = () => {
  ...
  const exportData = (fileSuffix = "adovation-applicants") => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axios.get(`${baseUrl}/adovations/all`);
        setApplications(res.data || []);
      } catch (err) {
        console.error("Failed to fetch Adovation applications", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className={styles.Applications}>
      <Title title="Adovation Applications" />
      <div className={styles.applications}>

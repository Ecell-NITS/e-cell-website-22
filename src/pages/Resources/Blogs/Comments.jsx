...
const Comments = () => {
  ...
  const isCommentFilled = () => {
    return comment !== "" && commentpic !== "" && commentauthor !== "";
  };
  ...
  return (
    <>
      <div className="comment-main">
        <div className="photoandnamediv">
          ...
          <div className="name-holder-loggedin">
            {isLoggedIn ? (
              <h2 onClick={handleGoToDashbaord} className="classofnameloggedin">
                {name}
              </h2>
            ) : (
              <h2 onClick={handleGoToDashbaord} className="classofnameloggedin">
        ...
        <textarea
          className="place-ka-holder-commnt"
        ...
        <input
          ...
          type="text"
          value={commentpic}
          onChange={handleCommentAuthorPic}
        />

        <div className="btnscommentandcancel">
          <button
            id="nhilikhnabhaicomment"
            onClick={handleCancel}
            disabled={!isCommentFilled() || disable}
            style={{ cursor: !isCommentFilled() || disable ? "not-allowed" : "pointer" }}
          >
            Cancel
          </button>
          ...
          <button
            ...
            disabled={!isCommentFilled() || disable}
            style={{
              cursor: !isCommentFilled() || disable ? "not-allowed" : "pointer",
              opacity: !isCommentFilled() || disable ? 0.5 : 1,
            }}
          >
            {submit ? "Submitting..." : "Comment"}
          </button>
        </div>
      </div>
      ...
      <div className="inctructionhbhaipadhlo">
        {instruction && (
          <span id="strict-instruction">
            You must be{" "}
            <span onClick={handleLoginPage} style={{ cursor: "pointer" }}>
              logged in
            </span>{" "}
            to comment.
          </span>
        ...
        <span id="strict-instruction">
          {" "}
          Please be respectful and avoid usage of any offensive words.
        </span>
      </div>
